...
A transient, user-initiated **what-if** derivation: "if this area were closed, what would break?" Architecturally the sibling of a [Projection](#projection) — it lives entirely **outside** the Anomaly→Incident pipeline. The distinction worth naming: a Projection extrapolates from a **real detected** disruption (an Incident); a Scenario **invents the premise** — a closure that is *not* happening — which makes it *more* hypothetical and sharpens the must-read-as-what-if requirement. A Scenario produces **no Anomaly**, creates **no Incident, Inbox row, or timeline entry**, makes no new feed calls, and is never persisted to a Recording; its blast radius is recomputed transiently against the live observation buffer and retracts completely on exit, leaving the live picture untouched (see [ADR 0005](docs/adr/0005-projections-transient-outside-anomaly-pipeline.md)).

Two independent "not real" axes apply: **hypothesis vs. observation** (true of *every* Scenario — drives the hedged wording and visual distinction) and **demo vs. user-created** (the `demo` flag — true only for presets like "close Slussen", labelled like an Injected Incident). The area is a bounding **box** (not a free polygon), so "affected" is a point-in-bbox test. Like every derivation it is auditable: affected vehicles trace to actual buffer observations; the area is computed from structured inputs, never free-written. A persistent scenario-active banner with one-click exit keeps a simulation from being mistaken for live reality — the same prominence rule as Replay's past-mode indicator.
