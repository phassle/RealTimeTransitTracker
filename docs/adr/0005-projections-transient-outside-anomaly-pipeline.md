...
- **Confidence floor — silence over a guess.** Each affected `(line, direction)` carries a 0–1 `confidence` (slice 4), a weighted score over: direction known (the dominant signal), the count of *fresh* downstream observations across the buffer window (≥2 ⇒ full credit, 1 ⇒ half, via `OBSERVATION_FRESH_MS`), and the recency of the most recent such observation. Below the named `CONFIDENCE_FLOOR` the entry is suppressed; with none surviving the whole Projection is `null` — the app stays silent rather than guessing (stories 5/6). The score is surfaced coarsely (`confidenceLabel`: high / medium); below-floor never reaches the UI.
